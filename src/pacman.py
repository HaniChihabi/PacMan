...
class Ghost:
        ...
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                ...
                if self.target[1] > self.y_pos and self.turns[3]:
                    ...
                    self.direction = 3       # what if he goes up and down and back up again
                    self.y_pos += self.speed
                elif self.turns[2]:
                    self.direction = 2
                    self.y_pos -= self.speed
            ...
            if self.target[1] > self.y_pos and self.turns[3]:        # Default move
                self.direction = 3
            elif self.target[0] < self.x_pos and self.turns[1]:
                self.x_pos -= self.speed
            ...
            if self.target[0] < self.x_pos and self.turns[1]:        # Default move
                self.direction = 1
                self.x_pos -= self.speed
            elif self.target[1] < self.y_pos and self.turns[2]:
                self.direction = 2
            ...
            if self.target[1] > self.y_pos and self.turns[3]:
                ...
                if self.target[0] > self.x_pos and self.turns[0]:        # Default move
                    self.direction = 0
                    self.x_pos += self.speed
                elif self.target[0] < self.x_pos and self.turns[1]:
                    self.direction = 1
    ...
    def move_pinky(self):
        # R, L U D
        # Pinky is going left or right whenever advantageoust, but only up or down on collision     
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
            elif not self.turns[0]:
...
def draw_misc():
    ...
    if game_over:
        ...
        gameover_text = font.render('Game Over! Enter to restart', True, 'red')
        screen.blit(gameover_text, (100, 300))
    if game_won:
        pygame.draw.rect(screen, 'white', [50, 200, 800, 300], 0, 10)
        pygame.draw.rect(screen, 'dark grey', [50, 220, 760, 260], 0, 10)
        gamewon_text = font.render('Victory! Enter to restart', True, 'green')
